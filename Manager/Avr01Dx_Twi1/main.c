...
FILE *uart1; //debug port, I connect to it with $picocom -b 38400 /dev/ttyUSB2
...
void testSlave() {
    ...
    if (twim_cb_interlock) { // this interlock tells if twi_started_at is safe to use
        ...
        if ( kRuntime > twi_ttl) 
        {
            ...
            twim_off();
            twim_callback(NULL);
            fprintf_P(uart1,PSTR("twi0 timed out\r\n"));
            while(!uart1_availableForWrite()); // block until uart is done
    ...
    if (uart1_availableForWrite()) { // if UART is done then print latest twi stats (but don't block).
        if (print_part == 0) {
            ...
            sei();
            fprintf_P(uart1,PSTR("ticks %lu, "), cp_twim_cb_last_elapsed);
            print_part = 1;
        } else if (print_part == 1) {
            fprintf_P(uart1,PSTR("tmo %u, "),  cp_twim_cb_timout_count);
            print_part = 2;
        } else if (print_part == 2) {
            fprintf_P(uart1,PSTR("short %u, "),  cp_twim_cb_elapsed_lessthan_delay);
            print_part = 3;
        } else if (print_part == 3) {
            fprintf_P(uart1,PSTR("good %u, "), cp_twim_cb_good_count);
            print_part = 4;
        } else if (print_part == 4) {
            fprintf_P(uart1,PSTR("bad %u\r\n"), cp_twim_cb_bad_count);
            print_part = 0;
