...
void Id(FILE *stream, const char name[])
{ 
    // /id? 
    if ( (command_done == 10) && (arg_count == 0) )
    {
        fprintf_P(stream, PSTR("{\"id\":{"));
        command_done = 11;
    }
    // /id? name 
    ...
    {
        fprintf_P(stream, PSTR("\"name\":\"%s\"" ),name);
        if (arg_count == 1) 
        { 
            command_done = 15;  
        }
    ...
    {
        fprintf_P(stream, PSTR("\"desc\":\"MacGyver (19260^1) " ));
        command_done = 13;
    }
    else if ( command_done == 13 )
    {
        fprintf_P(stream, PSTR("Board /w AVR128DA28\""));
        if (arg_count == 1) 
        { 
            command_done = 15; 
        }
    ...
    {
        fprintf_P(stream, PSTR("\"avr-gcc\":\"%s\""),__VERSION__);
        command_done = 15; 
    }
    ...
    {
        fprintf_P(stream, PSTR("{\"err\":\"idBadArg_%s\"}\r\n"),arg[0]);
        initCommandBuffer();
    }
}
