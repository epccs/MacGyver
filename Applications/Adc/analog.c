...
void Analog(unsigned long serial_print_delay_milsec)
{
    ...
    else if ( (command_done == 20) )
    {
        ...
        float temp_adc = adcAtomic((ADC_CH_t) arg_indx_channel);
        float temp_ref = *(calMap[arg_indx_channel].ref); // does the member operator have higer precedence, I don't recall that sort of stuff.
