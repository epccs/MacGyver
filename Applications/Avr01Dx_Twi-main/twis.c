...
static void address1        (uint8_t v) { TWI0.SADDR = (v<<1)|1; } //gencall enabled, so check address in callback
static void address2        (uint8_t v, bool nomask) { TWI0.SADDRMASK = (v<<1)|nomask; }
static void off             () { TWI0.SCTRLA &= ~1; }
static void on              () { TWI0.SCTRLA |= 1; }
static uint8_t   read            () { return TWI0.SDATA; }
static void write           (uint8_t v) { TWI0.SDATA = v; }
static void irqAllOn        () { TWI0.SCTRLA |= 0xE0; } // TWI_DIEN_bm | TWI_APIEN_bm | TWI_PIEN_bm
static void irqAllOff       () { TWI0.SCTRLA &= ~0xE0; }
static uint8_t   status          () { return TWI0.SSTATUS; }
