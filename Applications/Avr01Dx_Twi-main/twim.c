...
static void address         (uint8_t v) { off(); TWI0.MADDR = v<<1; } //off so no start produced
...
static void write           (uint8_t v) { TWI0.MDATA = v; }
static uint8_t   read            () { return TWI0.MDATA; }
static uint8_t   status          () { return TWI0.MSTATUS; }
